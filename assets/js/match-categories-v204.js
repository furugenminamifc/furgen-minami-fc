...
(function(){
 ...
 window.renderRecordInputs=function(){if(!isStaff())return;installUI();const chosen=$('matchCategory')?.value||'',existing=new Map(records.filter(r=>String(r.match_id)===String(editingMatchId)).map(r=>[String(r.player_id),r]));const list=players.filter(p=>(p.status==='現役'||existing.has(String(p.id)))&&(!chosen||playerCat(p.grade)===chosen||existing.has(String(p.id))));$('recordInputs').innerHTML=`<div class="category-entry-guide">${chosen?`<b>${chosen}</b> の選手を表示中（${list.length}名）`:'先にカテゴリーを選択してください。'}</div><div class="player-entry muted"><div>選手</div><div>出場</div><div>時間</div><div>得点</div><div class="extra">アシスト</div><div class="extra">黄</div><div class="extra">赤</div><div class="extra">MVP</div></div>`+(chosen?list.map(p=>{const r=existing.get(String(p.id))||{};return `<div class="player-entry" data-player="${p.id}"><div><b>${esc(p.name)}</b><div class="muted">${esc(playerCat(p.grade))} ${esc(p.position||'')}</div></div><div><input class="played" type="checkbox" onchange="renderMatchEvaluationInputs()" ${r.played?'checked':''}></div><div><input class="minutes" type="number" min="0" value="${r.minutes||0}"></div><div><input class="goals" type="number" min="0" value="${r.goals||0}"></div><div class="extra"><input class="assists" type="number" min="0" value="${r.assists||0}"></div><div class="extra"><input class="yellow" type="number" min="0" value="${r.yellow||0}"></div><div class="extra"><input class="red" type="number" min="0" value="${r.red||0}"></div><div class="extra"><input class="mvp" type="checkbox" ${r.mvp?'checked':''}></div></div>`}).join(''):'')};
