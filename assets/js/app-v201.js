...
async function loadAll(){
 ...
 applyTeamSettings();refreshFilters();renderAll();renderBirthdayNotice();renderStaffBirthdayList()
}
...
/* Ver.20 stable boot */
window.V20_BUILD = "20.1.0-20260731";
async function v20SafeBoot(){
  try {
    if ('serviceWorker' in navigator) {
      ...
      await navigator.serviceWorker.register('./sw-v201.js?build=20.1.0-20260731', {scope:'./'});
    }
    localStorage.setItem('furugen_v20_build','20.1.0-20260731');
...
function renderStaffBirthdayList(){
  ...
  if(!rows.length){
    root.innerHTML='<div class="birthday-empty">監督・コーチの誕生日はまだ登録されていません。</div>';
    return;
  }
