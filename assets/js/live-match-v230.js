...
function setup(){
  ...
  return shell('Ver.23.1 試合会場モード 完全完成版',0,
    '<div class="m230-grid">'+
...
function beginLineup(){
  ...
  state={
    version:'23.1',phase:'lineup',date:today(),
...
async function saveResult(){
  ...
  try{
    var mr={match_date:state.date,category:state.category,competition:state.competition||'',opponent:state.opponent,venue:state.venue||'',goals_for:state.gf,goals_against:state.ga,season:Number(state.date.slice(0,4)),memo:'試合会場モード Ver.23.1から登録',created_by:session.user.id};
