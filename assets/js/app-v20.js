...
/* Ver.20 stable boot */
window.V20_BUILD = "20.0.0-20260731";
async function v20SafeBoot(){
  try {
    if ('serviceWorker' in navigator) {
      const regs = await navigator.serviceWorker.getRegistrations();
      for (const r of regs) {
        if (!String(r.active?.scriptURL || '').includes('sw-v20.js')) await r.unregister();
      }
      await navigator.serviceWorker.register('./sw-v20.js?build=20.0.0-20260731', {scope:'./'});
    }
    localStorage.setItem('furugen_v20_build','20.0.0-20260731');
