...
async function getProfileWithRetry(user){if(!user?.id)return null;for(let i=0;i<3;i++){try{const r=await sb.from('profiles').select('*').eq('id',user.id).maybeSingle();if(r.data)return r.data}catch(e){}if(i<2)await new Promise(resolve=>setTimeout(resolve,400*(i+1)))}try{const rr=await sb.auth.refreshSession();if(rr?.data?.session)session=rr.data.session;const currentUser=session?.user||user;const r=await sb.from('profiles').select('*').eq('id',currentUser.id).maybeSingle();if(r.data)return r.data}catch(e){}return null}
