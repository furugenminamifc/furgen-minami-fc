...
async function getProfileWithRetry(user){window.FURUGEN_AUTH_DIAG={sessionUserId:session?.user?.id||null,queryUserId:user?.id||null,email:user?.email||session?.user?.email||null,attempts:[]};if(!user?.id)return null;for(let i=0;i<3;i++){try{const r=await sb.from('profiles').select('*').eq('id',user.id).maybeSingle();window.FURUGEN_AUTH_DIAG.attempts.push({attempt:i+1,data:!!r.data,error:r.error?{message:r.error.message,code:r.error.code,details:r.error.details}:null});if(r.data){window.FURUGEN_AUTH_DIAG.profile={id:r.data.id,role:r.data.role,active:r.data.active};return r.data}if(r.error)console.warn('profile fetch error',r.error)}catch(e){window.FURUGEN_AUTH_DIAG.attempts.push({attempt:i+1,exception:String(e)});console.warn('profile fetch exception',e)}if(i<2)await new Promise(resolve=>setTimeout(resolve,400*(i+1)))}return null}
async function loadProfile(){profile=null;if(session){let user=session.user;try{const u=await sb.auth.getUser();if(u?.data?.user)user=u.data.user}catch(e){}profile=await getProfileWithRetry(user)}const staff=isStaff();$('mode').textContent=staff?`${displayAccountEmail(session.user.email)}（${profile.role==='admin'?'管理者':'コーチ'}）`:'保護者閲覧モード';$('loginOut').classList.toggle('hidden',!!session);$('loginIn').classList.toggle('hidden',!session);$('entryForm').classList.toggle('hidden',!staff);$('needLogin').classList.toggle('hidden',staff);$('addPlayerBtn').classList.toggle('hidden',!staff);if(session)$('loginWho').textContent=`${displayAccountEmail(session.user.email)} / 権限：${profile?.role||'未設定'}`;updateHomeStatsPermission();window.dispatchEvent(new CustomEvent('furugen-auth-updated',{detail:{staff:staff,role:profile?.role||null,email:session?.user?.email||null}}))}
...
function playerHistoryRecords(playerId){
  ...
  const rows=(Array.isArray(records)?records:[]).filter(function(r){
...
function v232TotalsFromHistory(p){
  const out={
    ...
    minutes:0,starts:0,mvp:0,historyCount:0
  };
  if(!p)return out;

  playerHistoryRecords(p.id).forEach(function(r){
...
function totals(p){return v232TotalsFromHistory(p)}
...
let playerRecordEditorPlayerId=null;
let playerRecordEditorRows=[];

function recordMatchInfo(record){
  const match=(Array.isArray(matches)?matches:[]).find(function(m){
    return String(m.id)===String(record.match_id);
  });
  return match||{
    id:record.match_id,
    match_date:'',
...
function playerRecordEditorTotals(rows){
  return rows.reduce(function(total,r){
    const played=Boolean(r.played===true||r.played===1||r.started||r.starter||Number(r.minutes||0)>0);
    if(played)total.apps+=1;
    total.goals+=Number(r.goals||0);
    total.assists+=Number(r.assists||0);
    total.minutes+=Number(r.minutes||0);
    total.yellow+=Number(r.yellow||0);
    total.red+=Number(r.red||0);
    total.mvp+=r.mvp?1:0;
    return total;
  },{apps:0,goals:0,assists:0,minutes:0,yellow:0,red:0,mvp:0});
}

function renderPlayerRecordEditor(){
  const body=document.getElementById('playerRecordEditorBody');
  const summary=document.getElementById('playerRecordEditorSummary');
  if(!body||!summary)return;

  const player=(Array.isArray(players)?players:[]).find(function(p){
    return String(p.id)===String(playerRecordEditorPlayerId);
  });
  const rows=playerRecordEditorRows;
  const total=playerRecordEditorTotals(rows);

  summary.innerHTML=
    '<strong>'+(player?.name||'選手')+'</strong><br>'+
    '出場 '+total.apps+'試合 / 得点 '+total.goals+
    ' / アシスト '+total.assists+
    ' / 時間 '+total.minutes+'分 / 警告 '+total.yellow+
    ' / 退場 '+total.red;

  if(!rows.length){
    body.innerHTML='<div class="record-editor-empty">この選手の試合記録はありません。</div>';
    return;
  }

  body.innerHTML=rows.map(function(r,index){
    const m=recordMatchInfo(r);
    const date=m.match_date||m.date||'日付なし';
    const opponent=m.opponent||'対戦相手なし';
    const competition=m.competition||m.tournament||'';
    return `
      <div class="record-editor-card" data-record-id="${escapeHtml(String(r.id||''))}">
        <div class="record-editor-match">
          <strong>${escapeHtml(date)} ${escapeHtml(opponent)}</strong>
          <span>${escapeHtml(competition)}</span>
        </div>
        <div class="record-editor-grid">
          <label>出場
            <select data-field="played">
              <option value="true" ${r.played?'selected':''}>出場</option>
              <option value="false" ${!r.played?'selected':''}>出場なし</option>
            </select>
          </label>
          <label>先発
            <select data-field="started">
              <option value="true" ${(r.started||r.starter)?'selected':''}>先発</option>
              <option value="false" ${!(r.started||r.starter)?'selected':''}>先発なし</option>
            </select>
          </label>
          <label>時間<input type="number" min="0" data-field="minutes" value="${Number(r.minutes||0)}"></label>
          <label>得点<input type="number" min="0" data-field="goals" value="${Number(r.goals||0)}"></label>
          <label>アシスト<input type="number" min="0" data-field="assists" value="${Number(r.assists||0)}"></label>
          <label>警告<input type="number" min="0" data-field="yellow" value="${Number(r.yellow||0)}"></label>
          <label>退場<input type="number" min="0" data-field="red" value="${Number(r.red||0)}"></label>
          <label>MVP
            <select data-field="mvp">
              <option value="true" ${r.mvp?'selected':''}>あり</option>
              <option value="false" ${!r.mvp?'selected':''}>なし</option>
            </select>
          </label>
        </div>
        <div class="record-editor-actions">
          <button type="button" class="primary" onclick="saveSinglePlayerRecord('${String(r.id)}',${index})">この記録を保存</button>
          <button type="button" class="danger" onclick="deleteSinglePlayerRecord('${String(r.id)}')">この記録を削除</button>
        </div>
      </div>`;
  }).join('');
}

async function openPlayerRecordEditor(playerId){
  if(!isStaff()){
    showMessage('選手別試合履歴の編集は管理者・コーチログインが必要です。');
    return;
  }
  playerRecordEditorPlayerId=playerId;
  ...
  document.getElementById('playerRecordEditorModal')?.classList.remove('hidden');
  renderPlayerRecordEditor();
}

function closePlayerRecordEditor(){
  document.getElementById('playerRecordEditorModal')?.classList.add('hidden');
  playerRecordEditorPlayerId=null;
  playerRecordEditorRows=[];
}

function readEditorCardValues(index){
  const cards=document.querySelectorAll('#playerRecordEditorBody .record-editor-card');
  const card=cards[index];
  if(!card)throw new Error('編集対象が見つかりません。');
  const get=function(field){
    return card.querySelector('[data-field="'+field+'"]')?.value;
  };
  return {
    played:get('played')==='true',
    started:get('started')==='true',
    minutes:Math.max(0,Number(get('minutes')||0)),
    goals:Math.max(0,Number(get('goals')||0)),
    assists:Math.max(0,Number(get('assists')||0)),
    yellow:Math.max(0,Number(get('yellow')||0)),
    red:Math.max(0,Number(get('red')||0)),
    mvp:get('mvp')==='true'
  };
}

async function saveSinglePlayerRecord(recordId,index){
  if(!isStaff())return;
  try{
    const values=readEditorCardValues(index);
    const payload={
    ...
    // startedカラムが存在する環境だけ更新。失敗時はstartedなしで再実行。
    let result=await sb.from('records').update({...payload,started:values.started}).eq('id',recordId).select();
    if(result.error){
      result=await sb.from('records').update(payload).eq('id',recordId).select();
    }
    if(result.error)throw result.error;

    showMessage('試合記録を保存しました。通算成績を再計算します。','ok');
    await loadAll();
    playerRecordEditorRows=(Array.isArray(records)?records:[])
      .filter(function(r){return String(r.player_id)===String(playerRecordEditorPlayerId)});
    renderPlayerRecordEditor();
  }catch(e){
    console.error(e);
    showMessage('試合記録の保存エラー：'+String(e.message||e));
  }
}

async function deleteSinglePlayerRecord(recordId){
  if(!isStaff())return;
  if(!confirm('この選手のこの試合記録だけを削除します。試合そのものや他の選手の記録は削除しません。よろしいですか？'))return;
  try{
    const result=await sb.from('records').delete().eq('id',recordId);
    if(result.error)throw result.error;
    showMessage('この選手の試合記録1件を削除しました。','ok');
    await loadAll();
    playerRecordEditorRows=(Array.isArray(records)?records:[])
      .filter(function(r){return String(r.player_id)===String(playerRecordEditorPlayerId)});
    renderPlayerRecordEditor();
    ...
    console.error(e);
    showMessage('試合記録の削除エラー：'+String(e.message||e));
  }
}
...
function closeRecordAdjustment(){
  document.getElementById('recordAdjustmentModal')?.classList.add('hidden');
}
...
window.openPlayerRecordEditor=openPlayerRecordEditor;
window.closePlayerRecordEditor=closePlayerRecordEditor;
window.saveSinglePlayerRecord=saveSinglePlayerRecord;
window.deleteSinglePlayerRecord=deleteSinglePlayerRecord;
window.addPlayerRecordAdjustment=addPlayerRecordAdjustment;
window.closeRecordAdjustment=closeRecordAdjustment;
window.savePlayerRecordAdjustment=savePlayerRecordAdjustment;
...
function openPlayerModal(id=''){const p=players.find(x=>String(x.id)===String(id)),pv=privateForPlayer(id);$('playerModalTitle').textContent=p?'選手編集':'選手追加';$('editPlayerId').value=p?.id||'';$('editPhotoData').value=p?.photo_url||'';$('playerPhotoPreview').src=p?.photo_url||defaultAvatar();$('editPhoto').value='';$('editName').value=p?.name||'';$('editNumber').value=p?.number||'';$('editGrade').value=p?.grade||'';$('editBirthDate').value=p?.birth_date||'';$('editPosition').value=p?.position||'';$('editDominantFoot').value=p?.dominant_foot||'';$('editHeight').value=p?.height_cm||'';$('editWeight').value=p?.weight_kg||'';$('editStatus').value=p?.status||'現役';$('editStrengths').value=p?.strengths||'';$('editDevelopmentGoal').value=p?.development_goal||'';$('editFatigue').value=pv.fatigue_level||3;$('editCondition').value=pv.condition_level||3;$('editCoachNote').value=pv.coach_note||'';$('editGuardianName').value=pv.guardian_name||'';$('editGuardianPhone').value=pv.guardian_phone||'';$('editGuardianEmail').value=pv.guardian_email||'';$('editEmergencyContact').value=pv.emergency_contact||'';const autoTotal=p?totals(p):{apps:0,goals:0,assists:0,yellow:0,red:0,historyCount:0,legacyApps:0};
const totalMap={lifetimeApps:autoTotal.apps,lifetimeGoals:autoTotal.goals,lifetimeAssists:autoTotal.assists,lifetimeYellow:autoTotal.yellow,lifetimeRed:autoTotal.red};
if($('lifetimeBreakdown'))$('lifetimeBreakdown').innerHTML=
'<strong>集計元：</strong>recordsテーブルのみ<br>'+
'<strong>出場：</strong><b>'+Number(autoTotal.apps||0)+'</b>試合<br>'+
'<strong>得点：</strong><b>'+Number(autoTotal.goals||0)+'</b><br>'+
'<strong>アシスト：</strong><b>'+Number(autoTotal.assists||0)+'</b>';
...
function video181ScrollTo(id){
 const e=document.getElementById(id);
 if(e)e.scrollIntoView({behavior:'smooth',block:'start'});
}
function video181ToggleAdvanced(btn){
 const area=document.getElementById('video181AdvancedArea');
 if(!area)return;
 const closed=area.classList.toggle('is-closed');
 btn.textContent=closed?'高度な分析を開く':'高度な分析を閉じる';
}
function video181UpdateFileName(input){
 const e=document.getElementById('video181FileName');
 if(!e)return;
 e.textContent=input?.files?.[0]?.name||'まだ動画は選択されていません';
}

function loadVideo16File(input){ video181UpdateFileName(input); video171Bind();
...
function video17ConsumeTracking(time,home,away,ball){
 ...
 video17Data.timeline.push({
  ...
  homePoints:home.map(p=>({x:p.x,y:p.y})),
  awayPoints:away.map(p=>({x:p.x,y:p.y})),
  ballPoint:ball?{x:ball.x,y:ball.y}:null
 });
...
let video171Bound=false;

function video171Video(){
 return $('video16Player')
}
function video171Format(sec){
 sec=Math.max(0,Number(sec)||0);
 const m=Math.floor(sec/60),s=Math.floor(sec%60),cs=Math.floor((sec-Math.floor(sec))*100);
 return `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}.${String(cs).padStart(2,'0')}`
}
function video171Bind(){
 const video=video171Video();if(!video||video171Bound)return;
 video171Bound=true;
 video.addEventListener('loadedmetadata',video171UpdateTimeline,{passive:true});
 video.addEventListener('timeupdate',()=>{
  video171UpdateTimeline();
  renderVideo171SyncedPitch();
  renderVideo171CurrentEvents()
 },{passive:true});
 video.addEventListener('play',()=>video171SetState('再生中','running'));
 video.addEventListener('pause',()=>video171SetState('停止中','paused'));
 video.addEventListener('ended',()=>video171SetState('再生終了','done'));
}
function video171SetState(text,kind=''){
 const e=$('video171SyncState');if(e){e.textContent=text;e.className=`video17-badge ${kind}`}
}
function video171Play(){
 const v=video171Video();if(!v||!v.src){showMessage('先に動画を読み込んでください。');return}
 video171Bind();v.play()
}
function video171Pause(){
 const v=video171Video();if(v)v.pause()
}
function video171Step(direction){
 const v=video171Video();if(!v||!v.src)return;
 v.pause();v.currentTime=Math.max(0,Math.min(v.duration||0,v.currentTime+(direction*(1/25))));
 video171UpdateTimeline();renderVideo171SyncedPitch();renderVideo171CurrentEvents()
}
function video171Jump(sec){
 const v=video171Video();if(!v||!v.src)return;
 v.currentTime=Math.max(0,Math.min(v.duration||0,v.currentTime+sec))
}
function video171SeekFromRange(value){
 const v=video171Video();if(!v||!v.duration)return;
 v.currentTime=Number(value)||0;
 video171UpdateTimeline();renderVideo171SyncedPitch();renderVideo171CurrentEvents()
}
function video171UpdateTimeline(){
 const v=video171Video(),range=$('video171TimelineRange');
 if(!v)return;
 if(range){range.max=Number.isFinite(v.duration)?v.duration:100;range.value=v.currentTime||0}
 const cur=$('video171CurrentLabel'),dur=$('video171DurationLabel');
...
function video171Rows(){
 ...
 const overlay=typeof video16Read==='function'?video16Read():[];
 overlay.forEach(x=>{if((x.time||0)>=start&&(x.time||0)<=current+.15)rows.push({...x,source:'overlay'})});
 const tl=(video17Data?.timeline||[]);
 tl.forEach(x=>{if((x.time||0)>=start&&(x.time||0)<=current+.15){
  (x.homePoints||[]).forEach(p=>rows.push({type:'homeAuto',x:p.x,y:p.y,time:x.time}));
  (x.awayPoints||[]).forEach(p=>rows.push({type:'awayAuto',x:p.x,y:p.y,time:x.time}));
  if(x.ballPoint)rows.push({type:'ballAuto',x:x.ballPoint.x,y:x.ballPoint.y,time:x.time})
 }});
 return rows.sort((a,b)=>(a.time||0)-(b.time||0))
}
function renderVideo171SyncedPitch(){
 const pitch=$('video171Pitch');if(!pitch)return;
 const rows=video171Rows(),html=[video171PitchBase()];
 const showBall=$('video171ShowBallTrail')?.checked,showPlayer=$('video171ShowPlayerTrail')?.checked,showPass=$('video171ShowPasses')?.checked,showShot=$('video171ShowShots')?.checked;

 const ballRows=rows.filter(x=>['ball','ballAuto'].includes(x.type));
 if(showBall&&ballRows.length>1){
  html.push(`<svg class="v171-svg" viewBox="0 0 100 100" preserveAspectRatio="none"><polyline class="v171-ball-trail" points="${ballRows.map(x=>`${x.x},${x.y}`).join(' ')}"/></svg>`)
 }
 if(showPlayer){
 ...
 if(showPass){
  rows.filter(x=>x.type==='pass').forEach(x=>html.push(video171Line(x,'pass')))
 }
 if(showShot){
  rows.filter(x=>x.type==='shot').forEach(x=>html.push(video171Line(x,'shot')))
 }
...
function video171Line(x,kind){
 const dx=x.x2-x.x1,dy=x.y2-x.y1,len=Math.sqrt(dx*dx+dy*dy),angle=Math.atan2(dy,dx)*180/Math.PI;
 return `<div class="v171-line ${kind}" style="left:${x.x1}%;top:${x.y1}%;width:${len}%;transform:rotate(${angle}deg)"><i></i></div>`
}
function renderVideo171CurrentEvents(){
 const box=$('video171CurrentEvents');if(!box)return;
 const current=video171Video()?.currentTime||0,rows=(typeof video16Read==='function'?video16Read():[]).filter(x=>Math.abs((x.time||0)-current)<=1.0);
 if(!rows.length){box.innerHTML='<p class="muted">この時間帯のイベントはありません。</p>';return}
 box.innerHTML=rows.map(x=>{
  const p=players.find(a=>String(a.id)===String(x.player_id));
  const labels={ball:'ボール',player:'選手位置',pass:'パス',shot:'シュート',heat:'ヒート位置'};
  return `<div class="video171-event"><b>${video171Format(x.time)}</b><span>${labels[x.type]||x.type}</span><em>${esc(p?.name||'チーム')}</em><small>${esc(x.note||'')}</small></div>`
 }).join('')
}
function video171CreateEventFromCurrent(){
 const v=video171Video();if(!v||!v.src){showMessage('先に動画を読み込んでください。');return}
 const t=$('video16CurrentTime');if(t)t.textContent=formatVideo16Time(v.currentTime);
 const note=$('video16OverlayNote');if(note&&!note.value)note.value=`${video171Format(v.currentTime)} の場面`;
 showMessage('現在時間を分析入力へ反映しました。コート上をクリックして記録してください。','ok')
}
function video171FitToCurrent(){
 video171UpdateTimeline();renderVideo171SyncedPitch();renderVideo171CurrentEvents()
}
...
/* =========================================================
   Ver.20.0 完全版 AI分析・PWA・自動バックアップ
========================================================= */
...
function v191GenerateDailyPlan(){
  ...
  const report=[
    '【古堅南FC Ver.20.0 完全版 本日のAI監督レポート】',
