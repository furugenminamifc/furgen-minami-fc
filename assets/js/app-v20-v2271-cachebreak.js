...
async function getProfileWithRetry(user){if(!user?.id)return null;for(let i=0;i<3;i++){try{const r=await sb.from('profiles').select('*').eq('id',user.id).maybeSingle();if(r.data)return r.data;if(r.error)console.warn('profile fetch error',r.error)}catch(e){console.warn('profile fetch exception',e)}if(i<2)await new Promise(resolve=>setTimeout(resolve,400*(i+1)))}return null}
